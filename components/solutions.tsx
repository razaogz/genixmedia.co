...
const SOLUTION_CARDS = [
  {
    ...
    desc: 'Establish or expand a strong, cohesive digital footprint across platforms, channels, and assets.',
    href: '/solutions/growth',
    accent: 'from-[#a855f7]/20',
  },
  {
    icon: Shield,
    ...
    <SectionReveal delay={index * 0.1} className="h-full">
      <Link href={card.href} className="group block h-full">
        <motion.div
          ...
          className="relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] p-7 backdrop-blur-xl transition-colors duration-500 hover:border-[#a855f7]/40"
        >
          <div className="pointer-events-none absolute -inset-px rounded-2xl opacity-0 transition-opacity duration-500 group-hover:opacity-100">
            <div
              className={`absolute inset-0 rounded-2xl bg-[radial-gradient(circle_at_50%_0%,${card.accent},transparent_70%)]`}
            />
          </div>

          <div className="relative flex items-center justify-between">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-white/70 transition-colors duration-500 group-hover:border-[#a855f7]/40 group-hover:text-white">
              <Icon size={22} />
            </div>
            <span className="font-display text-3xl font-semibold leading-none text-white/10 transition-colors duration-500 group-hover:text-[#a855f7]/30">
          ...
          <div className="relative mt-6 flex items-center gap-2 text-sm font-medium text-white/70 transition-colors group-hover:text-white">
            Explore Solution
            <ArrowRight
              size={15}
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
          {hasWhatsapp && (
            <a
              ...
              className="group flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.03] px-6 py-3.5 text-sm font-medium text-white backdrop-blur-md transition-all hover:border-[#a855f7]/40 hover:bg-white/[0.06]"
            >
          ...
          <Link
            href="/contact"
            className="group relative overflow-hidden rounded-full bg-white px-6 py-3.5 text-sm font-semibold text-black shadow-[0_0_30px_rgba(168,85,247,0.35)] transition-opacity hover:opacity-90"
          >
